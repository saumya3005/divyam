...
export default function BookingsPage() {
  ...
  const myQuery = useGetMyBookings();
  const updateStatus = useUpdateBookingStatus();

  // Pick the right query based on role
  const isLoading = isAdmin ? adminQuery.isLoading : myQuery.isLoading;
  const isError = isAdmin ? adminQuery.isError : myQuery.isError;
  const bookings = isAdmin ? adminQuery.data : myQuery.data;
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-white">{isAdmin ? "All Bookings" : "My Bookings"}</h1>
        ...
        {isLoading ? (
            ...
            <table className="w-full text-left text-sm text-brand-gray">
              <thead className="bg-white/5 border-b border-white/5 uppercase text-xs">
                <tr>
                  ...
                  {isAdmin && <th className="px-6 py-4 font-medium text-white text-right">Actions</th>}
                </tr>
              </thead>
          ...
          >
            ...
            {!isAdmin && (
              <button 
                onClick={() => setIsFormOpen(true)}
                className="bg-brand-gold text-brand-dark px-5 py-2.5 rounded-lg font-medium text-sm hover:bg-brand-gold/90 transition-colors"
              >
