...
import { Loader2, Package, Plus, Eye, Edit2, Trash2, Search, AlertTriangle } from "lucide-react";
import { useGetInventory, useDeleteInventoryItem, InventoryItem } from "@/features/inventory/hooks/useInventory";
import { InventoryDrawerForm } from "@/features/inventory/components/InventoryDrawerForm";
import { toast } from "sonner";

const STATUS_COLORS: Record<string, string> = {
  Good: "bg-green-400/10 text-green-400 border-green-400/20",
  "Needs Repair": "bg-yellow-400/10 text-yellow-400 border-yellow-400/20",
  "In Maintenance": "bg-red-400/10 text-red-400 border-red-400/20",
};

const INVENTORY_CATEGORIES = ["Electronics", "Furniture", "Decor", "Catering", "Lighting", "Audio", "Miscellaneous"];
...
export default function InventoryPage() {
  ...
  const { data: inventoryData, isLoading, isError } = useGetInventory(debouncedFilters);
  ...
  const openDrawer = (mode: "create" | "edit" | "view", item: InventoryItem | null = null) => {
    setDrawerMode(mode);
        ...
        {isError ? (
            ...
            <table className="w-full text-left text-sm text-brand-gray whitespace-nowrap">
              <thead className="bg-white/5 border-b border-white/5 uppercase text-xs">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium text-white">Total Qty</th>
                  <th className="px-6 py-4 font-medium text-white">Available</th>
                ...
                {inventory.map((item) => (
                  <tr key={item._id} className="hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-4 font-medium text-white flex items-center gap-3">
                      ...
                      {item.name}
                      {item.availableQuantity <= item.quantity * 0.2 && item.availableQuantity > 0 && (
                        <span title="Low Stock"><AlertTriangle size={14} className="text-yellow-500 ml-2 shrink-0" /></span>
                      )}
                      {item.availableQuantity === 0 && (
                        <span title="Out of Stock"><AlertTriangle size={14} className="text-red-500 ml-2 shrink-0" /></span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={
                        item.availableQuantity === 0 
                          ...
                          : item.availableQuantity <= item.quantity * 0.2 
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 text-xs rounded-full border ${STATUS_COLORS[item.maintenanceStatus] || STATUS_COLORS.Good}`}>
                        {item.maintenanceStatus}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right space-x-2 sticky right-0 bg-brand-surface group-hover:bg-[#1a1a1a] transition-colors">
                      <button onClick={() => openDrawer("view", item)} className="text-brand-gray hover:text-white transition-colors" title="View"><Eye size={18} /></button>
                      <button onClick={() => openDrawer("edit", item)} className="text-blue-400 hover:text-blue-300 transition-colors" title="Edit"><Edit2 size={18} /></button>
                      <button onClick={() => { setItemToDelete(item._id); setDeleteModalOpen(true); }} className="text-red-400 hover:text-red-300 transition-colors" title="Delete"><Trash2 size={18} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-12 flex flex-col items-center text-center">
            <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-4">
              <Package size={28} className="text-brand-gray" />
            </div>
            <h3 className="text-white font-semibold text-lg mb-2">No Inventory Found</h3>
            <p className="text-brand-gray text-sm max-w-md mb-6">Adjust your filters or add new items to get started.</p>
          </motion.div>
        ) : null}
      </div>

      <InventoryDrawerForm isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} mode={drawerMode} item={selectedItem} />
        ...
        {deleteModalOpen && (
          <div className="fixed inset-0 z-100 flex items-center justify-center">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-brand-surface border border-white/10 rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl">
              ...
              <p className="text-brand-gray text-sm mb-6">Are you sure you want to remove this item? This action cannot be undone.</p>
