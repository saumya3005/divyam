...
const seedDB = async () => {
    try {
        ...
        const adminPassword = await (0, hash_service_1.hashPassword)("admin123");
        const userPassword = await (0, hash_service_1.hashPassword)("user123");
        ...
        const customers = await User_1.User.insertMany(Array.from({ length: 10 }).map((_, i) => ({
        ...
        const services = await Service_1.Service.insertMany(dummyServices);
